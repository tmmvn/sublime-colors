...
class CacheCleanupListener(sublime_plugin.EventListener):
    def on_close(self, view):
        # Called when a file is closed
        self.schedule_cleanup()

    def schedule_cleanup(self):
        # Use a timer to delay cleanup (avoids frequent calls)
        ...
        for filename in os.listdir(cache_dir):
            ...
            if os.path.isfile(file_path):
                ...
                if file_age > max_age_seconds:
                    try:
                        os.remove(file_path)
                        print("Cache cleanup: Removed {}".format(filename))
                    except OSError as e:
                        print("Cache cleanup: Error deleting {}: {}".format(filename, e))


def toicon(name, light=True):
    base_path = os.path.join(sublime.cache_path(), 'User', '%s.cache' % NAME)
...
def highlight_colors(view, selection=False, **kwargs):
    ...
    for i, col in enumerate(found):
        ...
        try:
            ...
            print(e)
            continue
        # Fix case when color it's the same as background color by adjusting
        # the color rgbs by 1. Note that his makes the color not a perfect
        # match for e.g. color picking
    ...
    for name, w in words.items():
        ...
        view.add_regions(name + '_icon', wi, '%sgutter' % prefix, icon=toicon(name), flags=sublime.PERSISTENT)
        all_regs.add(name)
    if not selection:
...
def get_delay(t, view):
    ...
    return delays #(min_delay, min_delay) if min_delay > delays[1] else delays
